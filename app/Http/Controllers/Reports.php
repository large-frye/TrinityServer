...
class Reports extends BaseController {
  ...
  const ALERT_FROM_INSPECTOR = 'alert_from_inspector';
      ...
      switch ($status) {
        ...
        case 'scheduled':
          ...
          $stringFields = array('Customer ID', 'Claim Num', 'Insured', 'City', 'State',
            'Adjuster', 'Insurance Company', 'Date of Inspection', 'Time of Inspection', 'Inspector', 'Inspection Type', 'Date Created');
          $fields = $this->createAssociateFieldArray($stringFields, $fields);
  ...
  private function getReportsByAttType($attentionType, $inspectionType) {
    ...
    if (gettype($attentionType) == 'array') {
      $query->where($attentionType[0], 1);
      $len = count($attentionType);
      for ($i = 1; $i < $len; $i++)
        $query->orWhere($attentionType[$i], 1);
    } else {
